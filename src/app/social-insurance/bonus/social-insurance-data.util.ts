...
/** 社会保険年度の開始月（4月）。yyyyMm が 2025-01 なら 2024-04 */
export function fiscalYearStartYyyyMm(yyyyMm: string): string {
  const { year, month } = parseYyyyMm(yyyyMm);
  if (month >= 4) {
    return `${year}-04`;
  }
...
export function isYyyyMmInFiscalYear(yyyyMm: string, fiscalStartYyyyMm: string): boolean {
  const fiscalEnd = addMonths(fiscalStartYyyyMm, 11);
  return yyyyMm >= fiscalStartYyyyMm && yyyyMm <= fiscalEnd;
}
