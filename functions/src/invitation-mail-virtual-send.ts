...
export const sendInvitationMailVirtual = onCall<SendInvitationInput>(
  {
  ...
  async (request) => {
    ...
    try {
      ...
      const from = '縄文 社会保険アプリ <onboarding@resend.dev>';
      const to = [email];
      const subject = `【重要】${tenantName} から社会保険管理システムへの招待`;
      ...
      await inviteRef.update({
        ...
        resendMessageId: mailRef.id ?? null,
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      });
